...
# -------------------------------------------------------------
# 1. SISMÓGRAFO EN EL CIELO (TRANSPARENTE + LÍNEA ROJA)
# -------------------------------------------------------------
...
# -------------------------------------------------------------
# 2. GRÁFICA FINAL DE LA SESIÓN AL SALIR
# -------------------------------------------------------------
def generar_grafica_5min(csv_path, session_start):
    ...
    # Color naranja para el pico máximo, azul para el resto
...
def main():
    ...
    print(f"🚀 Canoe Counter Arriondas [GPU: {device.upper()}] — Modo Directo")
    ...
    while True:
        ...
        results = model.predict(
            source=roi_river,
            classes=[8, 37],    # SOLO embarcaciones (ignora remeros)
            conf=0.15,
            imgsz=640,
            device=device,
            verbose=False
        ...
        for obj_id, (bx1, by1, bx2, by2), cx, cy in tracked_objects:
            ...
            if confirmada and (obj_id not in tracker.counted):
                ...
                with open(CSV_FILE, mode="a", newline="", encoding="utf-8") as f:
                    writer = csv.writer(f)
                    writer.writerow([now.strftime("%Y-%m-%d %H:%M:%S"), obj_id])
        ...
        # INSET PIP: ZOOM GRANDE (DOBLE DE TAMAÑO)
        # ---------------------------------------------------------
        pip_w = 480   # Duplicamos el ancho de 260 a 480px
        ...
        # Pegar el zoom sobre la zona del árbol
